...
from .config import system_splitter


# 切分数据工具类, 将dataset切分为多个可以切分的类别
class SplitDataset():
    # 指定训练集比例60%,测试及比例0.2
    def __init__(self, dataset_dir, saved_dataset_dir, train_ratio=0.6, test_ratio=0.2, show_progress=False):
        ...
        self.saved_train_dir = saved_dataset_dir + system_splitter + "train" + system_splitter
        self.saved_valid_dir = saved_dataset_dir + system_splitter + "valid" + system_splitter
        self.saved_test_dir = saved_dataset_dir + system_splitter + "test" + system_splitter
...
if __name__ == '__main__':
    split_dataset = SplitDataset(dataset_dir="original_dataset",  # 手动指定文件夹名字
