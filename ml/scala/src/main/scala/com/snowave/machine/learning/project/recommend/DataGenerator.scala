 ...
 * 模拟： 
 * 应用词表         applist.txt              app的基本信息表, 下载量是多少, 作者是谁等
 * 用户历史下载表   userdownload.txt         用户历史下载app
 * 正负例样本表     sample.txt               用户浏览app下载表, 记录了用户主动搜索某个app是否下载的信息
 * 每张表模拟10000 条数据 ，分别存入三个文件
 *
 * 用户搜索某个app就是感兴趣, 搜索之后下载与不下载就是二分类问题. 根据逻辑回归来获取两两app的关联重要程度
 *
 */
